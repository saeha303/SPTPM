...
def getKsp(maxper, event_instance_table, fineness, minsup, minoccur, granularity_of_G, pattern, maxPatternSize, minper = None):
    ...
    for count, season in start_times:
        max_count = len(season) - len(season) * minsup #minsup is a support given in decimal, less than 1
        cursor = 1
            ...
            for next_events in season[cursor:]:
                for event1time in range(events.early_start, events.latest_start, granularity_of_G):
                    for event2time in range(next_events.early_start, next_events.latest_start, granularity_of_G):
                        ...
                        idx_m = get_multiple(diff_list, diff)
                        idx_g = get_gcd(diff_list, diff)
                        ...
                        if idx_m == -1 and idx_g == -1: #if no multiplier or gcd exists
                            if diff > maxper*fineness*granularity_of_G: #diff is outside of periodicity
                                ...
                                diff_list.append([diff]) #just append
        ...
        for count, pers in all_sp_list:
            for all_diffs in pers:
                for diffs in all_diffs:
                    support = (len(diffs)+1)/total_support_ps[count]
                    if support >= minsup:
                        '''
                        improvement:
                        delete the following statements
                        keep the information (period, instances) of the sp pattern in a list
                        find if a better support variable is found to append the list
                        '''
                        sp_count += 1
                        break
                '''
                append the improvement list as value to all_sp_list, keeping cursor as key
                '''
    '''
    for each all_sp_list
    '''
